...
def decode(outputs, entities, length):
    ...
    for instance, ent_set, l in zip(outputs, entities, length):
        ...
        for cur in reversed(range(l)):
            ...
            #用于枚举出所有可能的路径组合
            #THW:node[pre]-->[(tail,type)]表示pre开头，tail结尾的实体及其类型
            #node[pre]-->{(head,tail):{next_index}}表示pre结点下一个节点next_index,存在于head开头tail结尾的实体中
            #每个节点对应一个token，且和此token相关的关系。每个关系中作为头结点出现。
...
def decodejump(outputs, entities, length):
    class Node:
        def __init__(self):
            self.THW = []                # [(tail, type)]
            self.NNW = defaultdict(set)   # {(head,tail): {next_index}}
        ...
        for cur in reversed(range(l)):
            ...
            for pre in range(cur+1):
                ...
                if instance[cur, pre] > 1: 
                    ...
                    heads.append(pre)
                # NNW
                if pre < cur and (instance[pre, cur] == 1 or instance[pre,cur]==2) :   
                    # cur node
                    for head in heads:
                        nodes[pre].NNW[(head,cur)].add(cur)
                    # post nodes
...
def decode_onlyjump(outputs, entities, length):
    class Node:
        def __init__(self):
            self.THW = []                # [(tail, type)]
            self.NNW = defaultdict(set)   # {(head,tail): {next_index}}
    ...
    for instance, ent_set, l in zip(outputs, entities, length):
        ...
        for cur in reversed(range(l)):
            ...
            for pre in range(cur+1):
                ...
                if instance[cur, pre] > 1: 
                    nodes[pre].THW.append((cur, instance[cur, pre]))
                    heads.append(pre)#尾巴所在的实体的头
                # NNW
                if pre < cur and instance[pre, cur] == 1  :   
                    # cur node
                    for head in heads:
                        nodes[pre].NNW[(head,cur)].add(cur)#从pre有个跳跃结点
                    # post nodes
                    for head,tail in nodes[cur].NNW.keys():
                        if tail >= cur and head <= pre:
                            nodes[pre].NNW[(head,tail)].add(cur)
...
def lossarea(outputs,len):
    for i in range(len):
        for j in range(len):
            if outputs[i][j]==1:
                loss1=(i**2+j**2)**0.5
            if outputs[i][j]>1:
                loss2=((i-1)**2+(j-1)**2)**0.5
    return print(loss1+loss2)